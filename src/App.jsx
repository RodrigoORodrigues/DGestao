    ...
    const parseSulAmericaExtrato = (blocosSulAmerica) => {
      for (let bloco of blocosSulAmerica) {
        try {
          ...
          if (matchComissao)
            comissao = parseFloat(
              matchComissao[1].replace(/\./g, "").replace(",", "."),
            );
          ...
          const matchData = bloco.match(
            /In[ií]cio de Vig[eê]ncia\s*:\s*(\d{2}\/\d{2}\/\d{4})/i,
          );
          if (matchData) {
          ...
          const matchDataPagamento = bloco.match(
            /Data pagamento\s*:\s*(\d{2}\/\d{2}\/\d{4})/i,
          );
          ...
          if (valorTotal > 0) {
            ...
            let calcParcela = calcularParcelaDaVigencia(
              inicioVigenciaDetectada,
              dataPagamentoDetectada || dataDeHojeInterna(),
            );
            if (
              !calcParcela &&
              !inicioVigenciaDetectada &&
