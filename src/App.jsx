...
export default function App() {
  ...
  const getFilteredVendas = () => {
    ...
    if (appliedVendasFilters) {
      ...
      if (f.loja !== "Todos")
        todasAsVendas = todasAsVendas.filter((v) => v.loja === f.loja);
      if (f.situacao !== "Todos")
        todasAsVendas = todasAsVendas.filter((v) => v.situacao === f.situacao);
      if (f.codigo)
        todasAsVendas = todasAsVendas.filter((v) =>
          (v.numero || "").toLowerCase().includes(f.codigo.toLowerCase()),
        );
      if (f.cliente)
        todasAsVendas = todasAsVendas.filter((v) =>
          (v.cliente || "").toLowerCase().includes(f.cliente.toLowerCase()),
        );
      if (f.contrato)
        todasAsVendas = todasAsVendas.filter((v) =>
          (v.contrato || "").toLowerCase().includes(f.contrato.toLowerCase()),
        );
      if (f.codigoOperadora)
        todasAsVendas = todasAsVendas.filter((v) =>
          (v.codigoOperadora || "")
            .toLowerCase()
            .includes(f.codigoOperadora.toLowerCase()),
        );
      if (f.notaFiscal)
        todasAsVendas = todasAsVendas.filter((v) =>
          (v.notaFiscal || "")
      ...
      if (f.vitalicio !== "Selecione")
        todasAsVendas = todasAsVendas.filter(
          (v) => v.vitalicio === f.vitalicio,
        );
      if (f.corretor !== "Todos")
        todasAsVendas = todasAsVendas.filter((v) => v.corretor === f.corretor);
      if (f.dataInicio)
        todasAsVendas = todasAsVendas.filter(
  ...
  const clientesFiltrados = clientes.filter((cli) => {
    const matchNome =
      cli.nome.toLowerCase().includes(filtroNomeCliente.toLowerCase()) ||
      (cli.documento && cli.documento.includes(filtroNomeCliente));
    const matchTipo =
      filtrosCli.tipo === "Todos" || cli.tipo === filtrosCli.tipo;
    const matchSituacao =
                        ...
                        {getAllVendas()
                          ...
                          .map((venda, idx) => (
                            ...
                            >
                              ...
                              <td className="py-3 px-4 font-medium text-emerald-600 dark:text-emerald-400 group-hover:text-emerald-700 dark:group-hover:text-emerald-300">
                                {new Intl.NumberFormat("pt-BR", {
                                  style: "currency",
                                  currency: "BRL",
                                }).format(venda.comissao || 0)}
                              </td>
              ...
              {hasAccess("vendas") && getAllVendas().length === 0 && (
                <div className="bg-slate-50 dark:bg-slate-800/50 border border-dashed border-slate-300 dark:border-slate-700 p-8 rounded-xl text-center text-slate-500 dark:text-slate-400">
                  ...
                  <p className="text-sm opacity-80">
                    Comece a adicionar vendas para ver o histórico aqui.
                  </p>
                  <button
                    onClick={() => setCurrentView("vendas")}
                    className="mt-4 bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded shadow-sm text-sm font-bold transition-colors"
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
                      ...
                      <div>
                        ...
                        >
                          ...
                          <option
                            value={`${nomeEmpresaUpper} SEGUROS`}
                          >{`${nomeEmpresaUpper} SEGUROS`}</option>
                          ...
                          {empresasList
                            ...
                            .map((e) => (
                              <React.Fragment key={e.id}>
                                <option
                                  value={`${e.nome.toUpperCase()} SEGUROS`}
                                >{`${e.nome.toUpperCase()} SEGUROS`}</option>
                      ...
                      pdfData.map((linha, idx) =>
                        editRowIndex === idx ? (
                          ...
                          >
                            ...
                            {reportTableCols.servico && (
                              <td className="py-1 px-2 border-r border-slate-200 dark:border-slate-700 text-center font-medium text-white text-[11px]">
                                {linha.servico || "-"}
                              </td>
                            )}
                            {reportTableCols.desconto && (
